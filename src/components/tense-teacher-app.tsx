...
export function TenseTeacherApp() {
  ...
  const handlePlayAudio = async (text: string, entryId: number) => {
    ...
    // Fallback: Cloudflare TTS (English or Arabic if browser TTS unavailable)
    if (audioRef.current) {
  ...
  const onSubmit = async (data: AhmedFormData | SaraFormData) => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "فشل الاتصال", description: `تعذر الاتصال بـ ${selectedTeacher}. يرجى المحاولة مرة أخرى.` });
      ...
      <Card className="w-full h-full max-w-3xl mx-auto my-4 shadow-xl bg-card rounded-lg flex flex-col min-h-0">
        <CardHeader className="p-4 border-b shrink-0">
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <Button variant="ghost" size="icon" onClick={() => setIsMuted(!isMuted)} aria-label={isMuted ? "إلغاء كتم الصوت" : "كتم الصوت"}>
