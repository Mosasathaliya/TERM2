 ...
 */
export async function chatStream(question: string): Promise<ReadableStream<string>> {
    const systemPrompt =
    ...
    return stream.pipeThrough(new TextEncoderStream()).pipeThrough(new TransformStream({
      transform(chunk, controller) {
        try {
          const json = JSON.parse(chunk.toString());
          if (json?.output?.text) {
              controller.enqueue(json.output.text);
          }
        } catch (e) {
