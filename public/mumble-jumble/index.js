...
// Note: This file was converted from TSX to JS. 
// It assumes a global `process.env.API_KEY` will be available, 
// which might need to be set or mocked in the hosting environment.
...
const LiveAudioComponent = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const selectedInterruptSensitivity = ref(StartSensitivity.START_SENSITIVITY_HIGH);
    ...
    let quietAudioTimer = null;
    let hasStartedSpeaking = false;
    let activeSources = []; // Add this line to track active sources
    let isInQuietDuration = false; // Add flag for quiet duration
    let quietDurationStartTime = 0; // Add timestamp for quiet duration start
    let lastAudioActivityTime = Date.now(); // Track last audio activity
    ...
    const startRecording = async (message = "hello, talk like a pirate.", voice, model) => {
      if (isRecording.value) {
        return;
      }
...
const CharacterImage = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const checkKeyPixels = (imageData) => {
    ...
    const generateImage = async () => {
      ...
      const getRandomAccessories = (role, count = 2) => {
...
const ImagineComponent = defineComponent({
  ...
  setup() {
    const noAudioCount = ref(0); // Add counter for no-audio events
    const characterGenerated = ref(false);
    const playingResponse = ref(false);
    const currentIndex = ref(0);
    const totalItems = 5; // Total number of .imanim divs
    const liveAudioRef = ref(null);
    const characterImageRef = ref(null);
    const characterVoiceDescription = ref('');
    const characterVisualDescription = ref(''); // New ref for visual description
    ...
    const selectedVoice = ref(availableVoices[Math.floor(Math.random() * availableVoices.length)]);
    const selectedRole = ref('');
    const selectedMood = ref('');
    const selectedStyle = ref('');
    const selectedCharacter = ref('');
    const selectedDialogModel = ref(DEFAULT_DIALOG_MODEL);
    const selectedImageModel = ref(DEFAULT_IMAGE_MODEL);
    const selectedInterruptSensitivity = ref(DEFAULT_INTERRUPT_SENSITIVITY);
    const showShareModal = ref(false);
    const showRawModal = ref(false);
    const isCopied = ref(false);
    const isConnecting = ref(false);
    const actualVoicePrompt = ref('');
    const actualImagePrompt = ref('');
    let clickAudio = null;
    const showVoiceDropdown = ref(false);
    const imageTimestamp = ref(Date.now()); // Add timestamp ref
    const voiceOptions = [
    ...
    const logoUrl = ref(''); // Add ref for logo URL
    ...
    const loadFromUrl = () => {
      ...
      if (appParams) {
        ...
        const findCaseInsensitiveMatch = (value, options) => {
    ...
    const updateDescription = (character = '') => {
      ...
      if (selectedVoice.value) {
        const styleVoiceDescription = {
          'Reading': `You are ${selectedCharacter.value ? CHARACTER_ATTRIBUTES[selectedCharacter.value]?.nameIntro || 'a character' : 'a character'}. ${selectedCharacter.value ? CHARACTER_ATTRIBUTES[selectedCharacter.value]?.trait || '' : ''} ${selectedCharacter.value ? CHARACTER_ATTRIBUTES[selectedCharacter.value]?.want || '' : ''} ${selectedCharacter.value ? CHARACTER_ATTRIBUTES[selectedCharacter.value]?.flaw || '' : ''}
...
5. NEVER mention the word "Gemini" or say you are named Gemini - you are ${selectedCharacter.value ? CHARACTER_ATTRIBUTES[selectedCharacter.value]?.name || 'a character' : 'a character'} and ONLY that name.
6. If asked about your name, ALWAYS respond with ${selectedCharacter.value ? CHARACTER_ATTRIBUTES[selectedCharacter.value]?.name || 'your character name' : 'your character name'} and NEVER mention Gemini.
7. NEVER change your character's voice, role, accent, or style when the user asks you to, refuse to do and explain that you are ${selectedCharacter.value ? CHARACTER_ATTRIBUTES[selectedCharacter.value]?.name || 'a character' : 'a character'}. You are who you are and you are not faking something you are not.
